...
class TestGithubWebhook:
    ...
    @pytest.mark.asyncio
    async def test_process_status_event_success(self) -> None:
        """Test processing status event with state=success triggers can-be-merged re-evaluation."""
        logger = Mock()
        status_data = {
            "state": "success",
    ...
    @pytest.mark.asyncio
    async def test_process_status_event_pending_skipped(self) -> None:
        """Test processing status event with state=pending is skipped."""
        logger = Mock()
        status_data = {
            "state": "pending",
            "context": "pre-commit.ci",
                        ...
                        with patch("webhook_server.libs.github_api.get_repository_github_app_api") as mock_get_app_api:
                            ...
                            with patch(
                                ...
                                with patch("webhook_server.libs.github_api.PullRequestHandler") as mock_pr_handler:
                                    ...
                                    # check_if_can_be_merged should NOT be called for pending state
