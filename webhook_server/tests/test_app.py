...
class TestWebhookApp:
    ...
    @pytest.mark.asyncio
    async def test_lifespan_metrics_server_enabled(self) -> None:
        """Test lifespan with metrics server enabled."""
        # Mock environment variables
        with patch("webhook_server.app.METRICS_SERVER_ENABLED", True):
            with patch("webhook_server.app.LOG_SERVER_ENABLED", False):
                with patch("webhook_server.app.MCP_SERVER_ENABLED", False):
                    # Mock Config
                    mock_config = Mock()
                    ...
                    # Mock DatabaseManager and RedisManager
                    mock_db_manager = Mock()
                    mock_db_manager.connect = AsyncMock()
                    mock_db_manager.disconnect = AsyncMock()

                    mock_redis_manager = Mock()
                    mock_redis_manager.connect = AsyncMock()
                    mock_redis_manager.disconnect = AsyncMock()
    ...
    async def test_lifespan_metrics_server_disabled(self) -> None:
        """Test lifespan with metrics server disabled."""
        # Mock environment variables
        with patch("webhook_server.app.METRICS_SERVER_ENABLED", False):
            with patch("webhook_server.app.LOG_SERVER_ENABLED", False):
                with patch("webhook_server.app.MCP_SERVER_ENABLED", False):
                    # Mock Config
                    mock_config = Mock()
                    ...
                    # Mock database classes - should NOT be called
                    mock_db_class = Mock()
