...
async def test_complete_pr_workflow_uses_graphql(mock_logger):
    ...
    # Step 1: Fetch PR
    pr_data = await api.get_pull_request(
        owner="test-owner",
        name="test-repo",
        number=123,
...
async def test_pr_workflow_with_error_recovery(mock_logger):
    ...
    pr_data = await api.get_pull_request(owner="test-owner", name="test-repo", number=123)
    assert pr_data["number"] == 123
    ...
    new_pr = await api.get_pull_request(owner="test-owner", name="test-repo", number=124)
...
async def test_context_manager_workflow(mock_logger):
    ...
    with (
        ...
        async with UnifiedGitHubAPI(token=TEST_GITHUB_TOKEN, logger=mock_logger) as api:
            # Fetch PR
            pr = await api.get_pull_request("test-owner", "test-repo", 456)
