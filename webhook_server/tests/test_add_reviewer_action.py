...
@pytest.mark.asyncio
async def test_add_reviewer_by_user_comment(caplog, process_github_webhook, pull_request):
...
@pytest.mark.asyncio
async def test_add_reviewer_by_user_comment_invalid_user(caplog, process_github_webhook, pull_request):
    process_github_webhook.repository = Repository()
    process_github_webhook.pull_request = PullRequest()
    issue_comment_handler = IssueCommentHandler(github_webhook=process_github_webhook)
